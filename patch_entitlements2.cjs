...
const targetStr = `<div style={{ background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: "16px", padding: "24px", boxShadow: "0 2px 4px rgba(0,0,0,0.02)" }}>
...
const entitlementUI = `
                ...
                <div style={{ background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '16px', padding: '24px', marginBottom: '24px', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
                  ...
                  <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                    <button onClick={handleSave} disabled={saving || !isDirty} style={{ padding: '10px 24px', background: !isDirty ? '#f1f5f9' : '#2563eb', color: !isDirty ? '#94a3b8' : '#fff', border: 'none', borderRadius: '8px', fontWeight: 600, cursor: !isDirty || saving ? 'not-allowed' : 'pointer' }}>
                      {saving ? 'Saving...' : 'Save Entitlements'}
                    </button>
                    {saveSuccess && <span style={{ color: '#10b981', fontWeight: 600, fontSize: '0.85rem' }}>Saved successfully!</span>}
                  </div>
                </div>
...
code = code.replace(targetStr, entitlementUI);
fs.writeFileSync(path, code, 'utf8');
console.log('Fixed Entitlements replacement');
